...
module Chess
  class Board
    ...
    alias color current_player_color
      ...
      pieces.each do |piece|
        if piece.name == :pawn
          ...
          if pawn_attack_vectors.any?
            pawn_attack_vectors.each do |v|
              @squares_under_attack[v] = { status: true, attacker: piece.name, apos: piece.current_position }
            end
          end
          next
        end
    ...
    def get_piece(position)
      ...
      if clear_destination?(position)
        raise "Invalid selection: #{position.to_algebraic}. You cannot select an empty square!"
      end
