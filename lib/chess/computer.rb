...
module Chess
  class Computer
    ...
    def initialize(name = 'Matricz', color)
      ...
      @selected_piece = nil
      %w[moves_buffer best_move_buffer best_attack_buffer].each { |v| instance_variable_set("@#{v}", {}) }
    end
      ...
      board.players_pieces.each do |piece|
        ...
        piece.safe_moves do |cboard, position|
          ...
          if !cboard.square_under_attack?(position) || sacrifice_worth?(piece, position)
            ...
            if cboard.players_pieces.reject do |p|
              p == piece
            end.none? { |ally| cboard.square_under_attack?(ally.current_position) }
              best_moves << position
            end
            safe_moves << position
          end
        ...
        enemies = safe_moves.select do |pos|
          board.is_a_piece?(pos)
        end.map { |pos| board.select_square(pos) }.sort.reverse

        unless enemies.empty?
          # most valuebale enemy
          mvenemy_position = enemies.first.current_position
          @best_attack_buffer[piece_position] = mvenemy_position if @best_attack_buffer.empty?
        ...
        best_moves = sort_moves(best_moves.compact).first unless best_moves.empty?
    ...
    def move(dictionary)
      ...
      sorted_hash = Hash[dictionary.sort_by { |key, _val| board.select_square(key) }]
    ...
    def sort_moves(moves)
      ...
      when :white
        moves.sort_by { |x, _y| x }.reverse!
      when :black
        moves.sort_by { |x, _y| x }
      end
    end
